class ApplicationController < ActionController::Base
  ...
  def build_meta_options(options)
    ...
    title = view_context.strip_tags(options[:title]) || current_organization.try(:title) || "세상을 바꾸는 시민들의 일상 정치 플랫폼, 빠띠 캠페인즈"
    ...
    description = view_context.strip_tags(options[:description]) || "세상을 바꾸는 시민들의 일상 정치 플랫폼 '빠띠 캠페인즈'입니다"
    {
      title:       title,
      reverse:     true,
      image:       image,
